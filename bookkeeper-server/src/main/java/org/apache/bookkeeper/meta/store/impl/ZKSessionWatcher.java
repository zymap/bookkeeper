 ...
 * under the License.
 */
package org.apache.bookkeeper.meta.store.impl;
...
public class ZKSessionWatcher implements AutoCloseable, Watcher {
    ...
    private synchronized void checkConnectionStatus() {
        try {
            CompletableFuture<Event.KeeperState> future = new CompletableFuture<>();
            zk.exists("/", false, (StatCallback) (rc, path, ctx, stat) -> {
                switch (KeeperException.Code.get(rc)) {
                case CONNECTIONLOSS:
                    future.complete(Event.KeeperState.Disconnected);
                    break;

                case SESSIONEXPIRED:
                    future.complete(Event.KeeperState.Expired);
                    break;

                case OK:
                default:
                    future.complete(Event.KeeperState.SyncConnected);
                }
            }, null);
            ...
            try {
                ...
                zkClientState = Event.KeeperState.Disconnected;
            }
    ...
    private void checkState(Event.KeeperState zkClientState) {
